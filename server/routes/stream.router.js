...
router.post('/bushel', async (req, res) => {
   ...
   try {
      for (let item of req.body.data) {
         ...
         } else if (item.contract.filled === true && foundContract.name === 'Signed' || item.contract.filled === true && foundContract.name === 'Delivered') {
            const queryText2 = `
               UPDATE "contract" SET "open_status" = (SELECT "id" FROM "contract_status" WHERE "name"='Delivered'), "quantity_fulfilled" = $2
               WHERE "bushel_uid" = $1;`;
            await pool.query(queryText2, [foundContract.bushel_uid, foundContract.contract_quantity]);
         ...
         } else if ((item.contract.quantity_submitted > 0) && foundContract.name === 'Signed') {
