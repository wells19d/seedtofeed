...
router.post('/create_NIR', rejectUnauthenticated, (req, res) => {
    ...
    const transaction = req.body.transType;
    ...
    pool.query(queryText, [field_id, oil, moisture, protein, energy, amino_acids, tested_at])
        ...
        "transaction_type") VALUES($1, Now(), 'NIR added', 'NIR added', $2) RETURNING *; `;

            pool.query(queryTransaction, [field_id, transaction])
                .then((result) => {
...
router.put('/update_NIR/', rejectUnauthenticated, (req, res) => {
    ...
    pool.query(queryText, [oil, moisture, protein, energy, amino_acids, NIRID])
        .then((result) => {
            ...
            "transaction_type") VALUES($1, Now(), 'NIR updated', 'NIR updated', (SELECT "name" FROM "transaction_type" WHERE "id" = 'contract')) RETURNING *; `;
